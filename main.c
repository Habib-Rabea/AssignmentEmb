...
int main()
{
	...
	Rcc_Init();
	Rcc_EnaAle(RCC_GPIOA);
	Gpio_ConfigPin(GPIO_A, 3, GPIO_INPUT, GPIO_PULL_UP);
	for (i = 5; i < 8; i++)
	{
	...
	while (1)
	{
		...
		if (Gpio_ReadPin(GPIO_A, 3) == 0)
		{
			for (i = 0; i <= 80000; i++)
				;
			if (Gpio_ReadPin(GPIO_A, 3) == 0 && flag == 0)
			{
				switch (status)
				{
				case 0:
